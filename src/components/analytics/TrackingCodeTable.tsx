  ...
  return (
    <section className="rounded-xl border border-border bg-card p-4">
      <div className="flex items-baseline justify-between mb-3">
        ...
        <span className="text-xs text-muted-foreground">광고비 · 리드 · 예약 · CPA · ROAS</span>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left text-muted-foreground">
              ...
              <th className="py-2 pl-3 font-medium text-right">ROAS(추정)</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 && (
            ...
            {rows.map((r) => (
              <tr key={r.trackingCode} className="border-b border-border/50 hover:bg-muted/30">
                ...
                <td className="py-2 px-3 text-right tabular-nums">{r.cpl > 0 ? fmtKRW(r.cpl) : '—'}</td>
                <td className="py-2 px-3 text-right tabular-nums">{r.cpa_reservation > 0 ? fmtKRW(r.cpa_reservation) : '—'}</td>
